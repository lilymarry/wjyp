...
- (CGRect)scanAreaRect{
    ...
    _scanAreaRect = CGRectMake(WIDTH(self) / 2 - self.scanAreaEdgeLength / 2,
                               HEIGHT(self) / 2 - self.scanAreaEdgeLength / 2 - 49,
...
- (void)startMoveLine{
    ...
    self.qrLine = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"ff_QRCodeScanLine_sub"]];
    ...
    animation.fromValue = @(self.scanAreaRect.origin.y + 20);
    animation.toValue = @(self.scanAreaRect.origin.y + self.scanAreaRect.size.height - 12 + 20);
...
- (void)addCornerLineWithContext:(CGContextRef)ctx rect:(CGRect)rect{
    ...
    //边角颜色
    CGContextSetRGBStrokeColor(ctx, 255 /255.0, 0/255.f, 0/255.f, 1);
    ...
    UILabel * thisTitle = [[UILabel alloc] initWithFrame:CGRectMake(0, rect.origin.y + rect.size.height, ScreenW, 30)];
    thisTitle.text = @"将二维码放入框内，即可自动扫描";
    thisTitle.font = [UIFont systemFontOfSize:13];
