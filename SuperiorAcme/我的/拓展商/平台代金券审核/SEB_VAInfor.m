...
#import "SEB_VAInfor.h"
#import "SEB_VAInforCell.h"
#import "SEB_VAInfor_top.h"
#import "SEV_window.h"

@interface SEB_VAInfor () <UITableViewDataSource,UITableViewDelegate>

@property (weak, nonatomic) IBOutlet UITableView *mTable;
@end

@implementation SEB_VAInfor
...
- (void)viewDidLoad {
    ...
    SEB_VAInfor_top * top = [[SEB_VAInfor_top alloc] initWithFrame:CGRectMake(0, 0, ScreenW, 340)];
    ...
    if ([_type isEqualToString:@"1"]) {
        top.status.text = @"审核中";
        top.statusContent.hidden = NO;
        top.oneBtn.hidden = YES;
        top.twoBtn.hidden = YES;
    }
    if ([_type isEqualToString:@"2"]) {
        top.status.text = @"待审核";
        top.statusContent.hidden = YES;
        top.oneBtn.hidden = NO;
        top.twoBtn.hidden = NO;
    }
