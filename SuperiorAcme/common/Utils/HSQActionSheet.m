...
@property (nonatomic, assign) HSQActionSheetStyle style;

@end

@implementation HSQActionSheet
...
#pragma mark - 初始化控件
- (instancetype)initWithTitle:(NSString *)title confirms:(NSArray *)confirms cancel:(NSString *)cancel style:(HSQActionSheetStyle)style
{
    self = [super init];
    if (self) {
        self.title = title;
...
- (void)createSubViews
{
    ...
    for (int i=0; i<self.confirms.count; i++) {
        ...
        if (self.style == HSQActionSheetStyleDestructive) {
            [confirm setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
        }else if(self.style == HSQActionSheetStyleCancel){
