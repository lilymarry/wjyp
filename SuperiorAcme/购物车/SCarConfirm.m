...
@property (strong, nonatomic) IBOutlet UIButton *shopCallBtn;
@property (strong, nonatomic) IBOutlet UIButton *callBtn;
@end

@implementation SCarConfirm
...
- (void)viewDidLoad {
    ...
    [_shopCallBtn addTarget:self action:@selector(shopCallBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    [_shopCallBtn setTag:0];
    [_callBtn addTarget:self action:@selector(shopCallBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    [_callBtn setTag:1];
...
- (void)showModel {
    if (_infor_isno == NO) {
        self.title = @"提交订单";
        [_car_img sd_setImageWithURL:[NSURL URLWithString:_model_car_img] placeholderImage:[UIImage imageNamed:@"无界优品默认空视图"]];
        _car_name.text = _model_car_name;
        _pre_money.text = [NSString stringWithFormat:@"￥%@",_model_pre_money];
        _all_price.text = [NSString stringWithFormat:@"车全款：￥%@",_model_all_price];
        _true_pre_money.text = [NSString stringWithFormat:@"可    抵：￥%@车款",_model_true_pre_money];
        ...
        _show_second_fiveView.hidden = YES;
        ...
        SCarOrderOrderInfo * infor = [[SCarOrderOrderInfo alloc] init];
        infor.order_id = _order_id;
        [MBProgressHUD showMessage:nil toView:self.view];
        [infor sCarOrderOrderInfoSuccess:^(NSString *code, NSString *message, id data) {
            [MBProgressHUD hideHUDForView:self.view animated:YES];
            if ([code isEqualToString:@"1"]) {
                
                SCarOrderOrderInfo * infor = (SCarOrderOrderInfo *)data;
                shop_name = infor.data.shop_name;
                lng = infor.data.lng;
                lat = infor.data.lat;
                ...
                [_car_img sd_setImageWithURL:[NSURL URLWithString:infor.data.car_img] placeholderImage:[UIImage imageNamed:@"无界优品默认空视图"]];
                _car_name.text = infor.data.car_name;
                _pre_money.text = [NSString stringWithFormat:@"￥%@",infor.data.pre_money];
                _all_price.text = [NSString stringWithFormat:@"车全款：￥%@",infor.data.all_price];
                _true_pre_money.text = [NSString stringWithFormat:@"可    抵：￥%@车款",infor.data.true_pre_money];
                ...
                _shop_name.text = infor.data.shop_name;
                _address.text = infor.data.address;
                _shop_mobile.text = infor.data.shop_mobile;
                _contact_name.text = infor.data.contact_name;
                _contact_mobile.text = infor.data.contact_mobile;
                shop_mobile = infor.data.shop_mobile;
                contact_mobile = infor.data.contact_mobile;
...
- (void)lefthBtnClick {
    if (self.SCarConfirm_Back) {
        self.SCarConfirm_Back();
    }
...
- (IBAction)submitBtn:(UIButton *)sender {
    ...
    payVC.model_type = @"汽车购";
    payVC.car_id = _model_car_id;
...
- (void)shopCallBtnClick:(UIButton *)btn {
    if (btn.tag == 0) {
        [[UIApplication sharedApplication]openURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@", [NSString stringWithFormat:@"tel://%@",shop_mobile]]] options:@{}completionHandler:^(BOOL success) {
        }];
    } else {
        [[UIApplication sharedApplication]openURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@", [NSString stringWithFormat:@"tel://%@",contact_mobile]]] options:@{}completionHandler:^(BOOL success) {
        }];
    }
}
