...
typedef void(^SAuctionOrderShoppingCartSuccessBlock) (NSString * code, NSString * message, id data);
typedef void(^SAuctionOrderShoppingCartFailureBlock) (NSError * error);

@interface SAuctionOrderShoppingCart : NSObject
@property (nonatomic, copy) NSString * auction_id;//    竞拍id    否    文本    1
@property (nonatomic, copy) NSString * order_id;//    订单id 1->竞拍付尾款需要传（一口价不需要传）
@property (nonatomic, copy) NSString * buy_type;//    购买类型：0->一口价；1->竞拍

@property (nonatomic, strong) SAuctionOrderShoppingCart * data;
