...
+(instancetype)cycleScrollViewWithFrame:(CGRect)frame shouldInfiniteLoop:(BOOL)infiniteLoop imageGroupsModels:(NSArray *)imageGroups  URLAttributeName:(NSString *)nameKey
{
    ...
    cycleScrollView.URLAttributeName=nameKey;
...
-(void)layoutSubviews
{
    ...
    self.collectionView.frame = CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height-35);
    self.pageControl.frame = CGRectMake(0, self.bounds.size.height - 35, self.bounds.size.width, 30);
    self.flowLayout.itemSize = CGSizeMake(_itemWidth, self.bounds.size.height-30);
...
//  NSString *imagePath = self.imgArr[itemIndex];
    [cell.imageView sd_setImageWithURL:[NSURL URLWithString:[self fetchModelUrl:itemIndex]] placeholderImage:[UIImage imageNamed:@"无界优品默认空视图_方形"]];
